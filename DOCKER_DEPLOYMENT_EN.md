...
Current compose setup uses local build for app services (`aitoearn-ai`, `aitoearn-server`, `aitoearn-web`) with `:local` tags.
Use `docker compose build` (or `docker compose up --build`) when code changes.
